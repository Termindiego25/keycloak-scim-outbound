...
public class ScimTargetProviderFactory
    ...
    /**
     * Reads a config value from the ComponentModel.
     * In production, getConfig() returns a MultivaluedHashMap populated by Keycloak.
     * In unit tests, getConfig() may be null (Mockito default); in that case we fall back
     * to ComponentModel.get(key) which is what test stubs typically wire up.
     */
    public static String get(ComponentModel m, String key, String def) {
        org.keycloak.common.util.MultivaluedHashMap<String, String> cfg = m.getConfig();
        String v = cfg != null ? cfg.getFirst(key) : m.get(key);
        return v != null ? v : def;
    }
}
